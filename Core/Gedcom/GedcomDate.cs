...
internal static class GedcomDate
{
  ...
  /// the original escaped form from, so it has to stay in residue even though <see cref="Parse"/> reads it.
  /// </summary>
  ...
  private static Date ParseFrenchRepublican(string value)
  {
    ...
    var hasDay = int.TryParse(tokens[0], out var day);
    var month = Array.IndexOf(FrenchRepublicanCalendar.MonthAbbreviations, tokens[1]);
    var hasYear = int.TryParse(tokens[2], out var year);
    if (!hasDay || !hasYear || month < 0)
      return unknown;

    var gregorian = FrenchRepublicanCalendar.ToGregorian(year, month + 1, day);
    return gregorian is null ? unknown : Date.Create(gregorian.Value);
  }
