...
try
{
  return await RunAsync(args, token);
}
catch (Exception ex)
{
...
static async Task<int> RunAsync(string[] args, CancellationToken token)
{
  if (args.Length == 0)
  {
    PrintUsage();
...
static async Task RunFindAsync(IProjectDocument document, string query, CancellationToken token)
{
...
static async Task RunRelativesAsync(IProjectDocument document, int personId, CancellationToken token)
{
...
static async Task RunTreeAsync(IProjectDocument document, int personId, CancellationToken token)
{
  var person = await document.Persons.TryGetPersonByIdAsync(personId, token);
  if (person is null)
  {
...
static string FormatDate(Date date) => date.Status switch
{
